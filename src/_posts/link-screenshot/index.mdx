  ...
  return (
    ...
    >
      {isHovering && linkScreenshot && (
        <div className="absolute z-10 block w-32 pointer-events-none right-1/2 lg:block bottom-[2.0rem] animate-fade-in-up">
          <Image
            src={linkScreenshot}
            height={180}
            width={300}
...
The image container `div` also has a custom class named `animate-fade-in-up` which is simply a custom animation defined in my `tailwind.config.js` which fades in the image nice and smoothly and looks like this:
