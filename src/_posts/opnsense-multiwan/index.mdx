...
Next, we'll add a firewall rule for the interface(s) that our target devices are on to send traffic to Tailscale Gateway if the target **is not** one of the above RFC1918 addresses
