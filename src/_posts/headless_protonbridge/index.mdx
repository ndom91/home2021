...
2. Application listens only on localhost (`127.0.0.1`)
   2a. And only on non-standard ports
...
You can see in my case, I'm using a Proxmox host that also needed to forward that port to a guest located at `10.0.1.52`, for example. Change that internal IP to wherever your `proton-bridge` service is running, or if its on the same machine just set it to `127.0.0.1` or `0.0.0.0`.
